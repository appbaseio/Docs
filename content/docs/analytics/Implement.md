...
You can disable this behavior in `ReactiveSearch` by defining the `appbaseConfig` prop in the `ReactiveBase` and set the `emptyQuery` as `false`. If you're not using the `ReactiveSearch`, then just don't send the `X-Search-Query` header while performing a `match_all` query.
