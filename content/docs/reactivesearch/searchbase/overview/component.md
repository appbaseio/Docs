...
The `Component` class represents a search component that can be used to build different kind of search components for examples, 
...
Here, we are specifying that the result component should update whenever one of the blacklist items is not present and simultaneously any one of city or topics matches.
...
-   **suggestions** `Array<Object>`
    To get the parsed suggestions from the `hits`. If `enableQuerySuggestions` property is set to `true` then the query suggestions will get append at the top with a top level property named `_query_suggestion` as `true`.
...
-   **triggerCustomQuery**
    `true` executes the query for the dependent components by `react` prop
-   **stateChanges**
