...
Once the pipeline is built, we can start using the data we get. We would use [`SearchBox`](/docs/reactivesearch/react/search/searchbox) component to search results and [`ReactiveList`](/docs/reactivesearch/react/result/reactivelist) component to display results. 

Our [`SearchBox`](/docs/reactivesearch/react/search/searchbox) component would look like below.
```js
<SearchBox 
    className="SearchBox" 
    autosuggest 
...
We would use use [`ReactiveList`](/docs/reactivesearch/react/result/reactivelist) as below. Note how we connect the [`ReactiveList`](/docs/reactivesearch/react/result/reactivelist) to watch on changes to [`SearchBox`](/docs/reactivesearch/react/search/searchbox) by using `react` property.
...
We would get data from `knowledgeGraph` inside of `rawData`, in the `render` method of [`ReactiveList`](/docs/reactivesearch/react/result/reactivelist). We would then render the results as below.
```js
<ReactiveList
    //...props
