...
-	`empty_query`: can be used for setting promoted values when no query is defined, i.e. an `<empty_query>` condition. This is often the case when an end user is opening the search UI for the very first time. This rule can be used for promoting a default set of results.
